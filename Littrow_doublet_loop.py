...
# Initialization
#D = 20                          # component diameter [mm]
#L = [400,550,700]               # wavelength [nm]
L = np.linspace(380,740,10)
l_avg = (380+740)/2
f_avg = 100                         # focal length of set [mm]
NA = 0.1                        # numerical aperature 
double = False
D = 2*f_avg*NA
# Figure 
plt.close('all')
...
for lensType in ['PCX_K','PCX_F','doublet']:
    f = []
    df = [] 
    for l in L: 
        ...
        if lensType == 'doublet':    
            R1 = f_avg*(2*calc_n_K(l_avg)-calc_n_F(l_avg)-1)
            R = [R1,-R1,0]
            d = [50,5,3,110]
            n = [(n0,n1),(n1,n2),(n2,n0)]
        elif lensType == 'PCX_K':
#            print((calc_n_K(l_avg)-1)*f_avg)
            R = [(calc_n_K(l_avg)-1)*f_avg,0]
            d = [50,8,110]
            n = [(n0,n1),(n1,n0)]
        elif lensType == 'PCX_F':
            R = [(calc_n_F(l_avg)-1)*f_avg,0]
            d = [50,8,110]
            n = [(n0,n2),(n2,n0)]
        ...
        for jdx,r_0 in enumerate(np.linspace(-(D/2),(D/2-2*D/10),20)):    # starting positions from axis
            z = []
            r = [r_0]
            theta = [0.03]     # incident angle of beam
        ...
        for j in range(jdx-1):
            for k in range(j+1,jdx):
                ...
                g += 1
        f.append(np.mean(cross))
        df.append(np.std(cross))
...
#        print(f'f = {f}, df = {df}')
    ...
    fig1 = plt.figure(2)
    ax1 = plt.subplot(111)
    h.append(plt.errorbar(L,f,yerr=df,capsize=5,elinewidth=3,fmt='.--',MarkerSize = 10))
...
plt.setp(h[2],Color='r')
plt.legend(['$\mathrm{PCX_K}$','$\mathrm{PCX_F}$','$\mathrm{Doublet}$'],fontsize=15)
plt.xlabel('$\lambda \mathrm{(nm)}$')
plt.ylabel('$f \  \mathrm{(mm)}$')
plt.tight_layout()